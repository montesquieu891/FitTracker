...
class TestC1MigrationSchema:
    ...
    EXPECTED_CORE_TABLES = [
        "users", "sponsors", "profiles", "tracker_connections",
        "activities", "point_transactions", "drawings", "prizes",
        "tickets", "prize_fulfillments",
    ]
    ...
    def test_indexes_include_users_email(self) -> None:
        ...
        email_indexes = [
            sql for sql in MIGRATION_002_INDEXES
    ...
    def test_run_migrations_twice_no_error(self) -> None:
        ...
        with patch("scripts.migrations.table_exists", return_value=True):
            ...
            mock_cursor.execute.side_effect = [
                alter_exc, alter_exc, alter_exc, alter_exc,  # 4 ALTER column
                alter_exc,  # balance version
                idx_exc, idx_exc, idx_exc, idx_exc, idx_exc, idx_exc, idx_exc, idx_exc,  # indexes
                idx_exc, idx_exc, idx_exc, idx_exc, idx_exc, idx_exc,  # CP7+CP8 indexes
                None,  # duality view
            ]
            actions2 = run_migrations(mock_conn)
